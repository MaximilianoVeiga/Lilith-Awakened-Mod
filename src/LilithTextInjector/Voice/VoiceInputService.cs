...
// Push-to-talk capture, Paraformer realtime ASR and HTTP transcription fallback.
internal static class VoiceInputService
{
    // Shared with Ui/DialogueManagerUpdatePatch.KeyBindings.cs, which resets this
    // flag whenever the push-to-talk hotkey is rebound.
    internal static bool KeyWasDown;
    ...
    internal static void HandleInput(DialogueManager manager)
    {
        var voiceInputKey = Plugin.VoiceInputKey.Value;
        var voiceInputKeyDown = IsKeyCurrentlyDown(voiceInputKey);
        var voiceInputKeyPressed = voiceInputKeyDown && !KeyWasDown;
        ...
        if (voiceInputKeyPressed)
        {
            ...
            var activeVoiceProvider = DialogueManagerUpdatePatch.NormalizeAiProvider(Plugin.AiProvider.Value);
            ...
            if (string.IsNullOrWhiteSpace(voiceApiKey))
            {
                ...
                manager.ForceSay(DialogueManagerUpdatePatch.LocalizedText($"還沒有設定 {providerName} API Key。", $"还没有设置 {providerName} API Key。", $"{providerName} APIキーがまだ設定されていないよ。", $"The {providerName} API key has not been set yet."), string.Empty, 6f);
                return;
            }
            try
            {
                ...
                manager.ForceSay(DialogueManagerUpdatePatch.LocalizedText("正在聽……", "正在听……", "聞いているよ……", "Listening…"), string.Empty, maxSeconds + 2f);
            ...
            catch (Exception exception)
            {
                ...
                manager.ForceSay(DialogueManagerUpdatePatch.LocalizedText("沒有收到麥克風的聲音。", "没有收到麦克风的声音。", "マイクの音が届いていないみたい。", "I didn't receive any microphone audio."), string.Empty, 6f);
            }
        }
    ...
    private static void StopVoiceInputRecording(bool reachedLimit)
    {
        ...
        try
        {
            ...
            var qwenVoiceInput = string.Equals(DialogueManagerUpdatePatch.NormalizeAiProvider(Plugin.AiProvider.Value), "Qwen", StringComparison.Ordinal);
    ...
    private static async Task<string> ReceiveParaformerTranscriptAsync(ClientWebSocket socket, CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            ...
            if (eventName == "task-finished")
                return DialogueManagerUpdatePatch.CleanReply(finalSentences.Count > 0 ? string.Concat(finalSentences) : latestInterim);
    ...
    {
        var baseUri = new Uri(DialogueManagerUpdatePatch.NormalizeQwenBaseUrl(Plugin.QwenBaseUrl.Value));
    ...
    {
        if (string.Equals(DialogueManagerUpdatePatch.NormalizeAiProvider(Plugin.AiProvider.Value), "Qwen", StringComparison.Ordinal))
        {
    ...
    private static async Task RequestQwenTranscriptionAsync(byte[] wav, string languageInstruction)
    {
        try
        {
            var baseUrl = DialogueManagerUpdatePatch.NormalizeQwenBaseUrl(Plugin.QwenBaseUrl.Value);
