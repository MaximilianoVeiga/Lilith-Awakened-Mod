...
// Application and game launching, including Start menu and shortcut resolution.
internal static class ApplicationLauncher
{
    ...
    internal static void LogOfficialCategories()
    {
        try
        {
    ...
    internal static bool TryLaunch(string text, out string reply)
    {
        reply = string.Empty;
        ...
        else if (Regex.IsMatch(text, "(記事本|记事本|メモ帳|notepad)", RegexOptions.IgnoreCase))
        {
            ...
            appName = DialogueManagerUpdatePatch.ApiKeyText("記事本", "记事本", "メモ帳", "Notepad");
        }
        else if (Regex.IsMatch(text, "(計算機|计算器|電卓|calculator|calc)", RegexOptions.IgnoreCase))
        {
            target = "calc.exe";
            arguments = string.Empty;
            appName = DialogueManagerUpdatePatch.ApiKeyText("計算機", "计算器", "電卓", "Calculator");
        }
        else if (Regex.IsMatch(text, "(檔案總管|文件资源管理器|資源管理器|エクスプローラー|file explorer|explorer)", RegexOptions.IgnoreCase))
        {
            target = "explorer.exe";
            arguments = string.Empty;
            appName = DialogueManagerUpdatePatch.ApiKeyText("檔案總管", "文件资源管理器", "エクスプローラー", "File Explorer");
        }
        else if (Regex.IsMatch(text, "(瀏覽器|浏览器|ブラウザ|browser|chrome|edge)", RegexOptions.IgnoreCase))
        {
            target = "https://www.google.com/";
            arguments = string.Empty;
            appName = DialogueManagerUpdatePatch.ApiKeyText("瀏覽器", "浏览器", "ブラウザ", "browser");
        }
        ...
        else
        {
            ...
            {
                reply = DialogueManagerUpdatePatch.ApiKeyText($"好，已切換到{appName}。", $"好，已切换到{appName}。", $"うん、{appName}に切り替えたよ。", $"Sure, I focused {appName}.");
            ...
            {
                reply = DialogueManagerUpdatePatch.ApiKeyText($"好，正在開啟{startApplication.Name}。", $"好，正在打开{startApplication.Name}。", $"うん、{startApplication.Name}を開くね。", $"Sure, I'm opening {startApplication.Name}.");
                return true;
            }
        ...
        try
        {
            ...
            reply = DialogueManagerUpdatePatch.ApiKeyText($"好，幫你開啟{appName}。", $"好，帮你打开{appName}。", $"うん、{appName}を開くね。", $"Sure, I'll open {appName}.");
            Plugin.PluginLog.LogInfo($"Launched allowlisted application target '{target}'.");
        }
        catch (Exception exception)
        {
            reply = DialogueManagerUpdatePatch.ApiKeyText($"{appName}沒有成功開啟。", $"{appName}没有成功打开。", $"{appName}を開けなかった……", $"I couldn't open {appName}.");
    ...
    internal static void EnsureFile()
    {
        try
        {
            ...
            if (File.Exists(ModDataPaths.ApplicationLauncherPath))
                return;
            var defaults = new[]
            {
                new ConfiguredApplication { Name = "記事本", Target = "notepad.exe", Aliases = new[] { "記事本", "记事本", "メモ帳", "notepad" } },
                new ConfiguredApplication { Name = "計算機", Target = "calc.exe", Aliases = new[] { "計算機", "计算器", "電卓", "calculator", "calc" } },
                new ConfiguredApplication { Name = "檔案總管", Target = "explorer.exe", Aliases = new[] { "檔案總管", "文件资源管理器", "資源管理器", "エクスプローラー", "file explorer", "explorer" } },
                new ConfiguredApplication { Name = "瀏覽器", Target = "https://www.google.com/", Aliases = new[] { "瀏覽器", "浏览器", "ブラウザ", "browser", "chrome", "edge" } },
                new ConfiguredApplication { Name = "Steam", Target = "steam://open/main", Aliases = new[] { "Steam", "蒸汽平台", "スチーム" } }
            };
            File.WriteAllText(ModDataPaths.ApplicationLauncherPath,
                JsonSerializer.Serialize(defaults, new JsonSerializerOptions
                {
                    WriteIndented = true,
                    Encoder = JavaScriptEncoder.UnsafeRelaxedJsonEscaping
                }), Encoding.UTF8);
            Plugin.PluginLog.LogInfo($"Created editable application launcher list at {ModDataPaths.ApplicationLauncherPath}.");
        }
        catch (Exception exception)
        {
    ...
    internal static ConfiguredApplication? FindConfiguredApplication(string text)
    {
        try
        {
            EnsureFile();
            var launchers = JsonSerializer.Deserialize<ConfiguredApplication[]>(File.ReadAllText(ModDataPaths.ApplicationLauncherPath),
                new JsonSerializerOptions { PropertyNameCaseInsensitive = true }) ?? Array.Empty<ConfiguredApplication>();
    ...
    internal static ConfiguredApplication? FindOfficialApplication(string text)
    {
        ...
        if (Regex.IsMatch(text, "(Spotify|スポティファイ|Spotify 音樂|Spotify 音乐)", RegexOptions.IgnoreCase))
        {
            ...
            return new ConfiguredApplication { Name = "Spotify", Target = shortcut ?? "spotify:" };
        }
        ...
        foreach (var application in applications)
        {
            ...
            foreach (var executable in application.Executables)
            {
                ...
                if (!string.IsNullOrWhiteSpace(resolved))
                    return new ConfiguredApplication { Name = application.Name, Target = resolved };
            }
            ...
            return new ConfiguredApplication { Name = application.Name, Target = application.Executables[0] };
        }
        return null;
    }

    private static ConfiguredApplication? FindOfficialGame(string text)
    {
        var games = new[]
        {
        ...
        foreach (var game in games)
        {
            ...
            if (!string.IsNullOrWhiteSpace(shortcut))
                return new ConfiguredApplication { Name = game.Name, Target = shortcut };
            if (!string.IsNullOrWhiteSpace(game.FallbackTarget))
                return new ConfiguredApplication { Name = game.Name, Target = game.FallbackTarget };
            Plugin.PluginLog.LogWarning($"Official game '{game.Name}' was requested but its launcher shortcut was not found.");
            return new ConfiguredApplication { Name = game.Name, Target = game.Name };
        }
        return null;
    }

    internal static string? ResolveWindowsShortcut(string[] names)
    {
    ...
    internal static string? ResolveFuzzyWindowsShortcut(string requestedName)
    {
        if (!OperatingSystem.IsWindows()) return null;
    ...
    internal static WindowsStartApplication? ResolveWindowsStartApplication(string requestedName)
    {
        if (!OperatingSystem.IsWindows()) return null;
        var requested = NormalizeApplicationName(requestedName);
        if (requested.Length < 2) return null;
    ...
    internal static bool TryFocusRunningApplication(string requestedName)
    {
        var requested = NormalizeApplicationName(requestedName);
    ...
    internal sealed class ConfiguredApplication
    {
