...
// Natural-language desktop commands: screenshots, folders, windows, media, clipboard.
internal static class DesktopCommandRouter
{
    internal static bool TryHandleScreenshotCommand(string text, out string reply)
    {
        reply = string.Empty;
    ...
    internal static bool TryHandleComputerCommand(string text, out string reply)
    {
        reply = string.Empty;
    ...
    internal static bool TryReportSystemStatus(string text, out string reply)
    {
        reply = string.Empty;
        if (Regex.IsMatch(text, "(電池|电池|電量|电量|battery|バッテリー).{0,12}(多少|幾|几|剩|狀態|状态|status|level|残|ある)|((多少|幾|几|剩).{0,8}(電量|电量|battery))", RegexOptions.IgnoreCase))
        {
            ...
            {
                reply = DialogueManagerUpdatePatch.ApiKeyText("這台電腦沒有回報可用的電池資訊。", "这台电脑没有报告可用的电池信息。", "このPCからバッテリー情報を取得できなかったよ。", "This PC did not report usable battery information.");
            }
            else
            {
        ...
        if (Regex.IsMatch(text, "((系統|系统|電腦|电脑|PC).{0,5}(記憶體|内存|memory|RAM|メモリ)|(記憶體|内存|memory|RAM|メモリ).{0,8}(使用|剩|狀態|状态|usage|status|空き))", RegexOptions.IgnoreCase))
        {
            ...
            if (!GlobalMemoryStatusEx(ref memory))
                reply = DialogueManagerUpdatePatch.ApiKeyText("沒有讀到記憶體狀態。", "没有读取到内存状态。", "メモリの状態を取得できなかった。", "I could not read the memory status.");
            else
            {
        ...
        if (Regex.IsMatch(text, "((系統|系统|C槽|C盘|磁碟|磁盘|硬碟|硬盘|disk|storage|ストレージ).{0,10}(空間|空间|剩|可用|free|available|空き))", RegexOptions.IgnoreCase))
        {
            ...
            catch (Exception exception)
            {
                ...
                reply = DialogueManagerUpdatePatch.ApiKeyText("沒有讀到系統磁碟狀態。", "没有读取到系统磁盘状态。", "システムドライブの状態を取得できなかった。", "I could not read the system-drive status.");
            }
            return true;
        }
        ...
        if (Regex.IsMatch(text, "((網路|网络|internet|network|インターネット|ネット).{0,10}(連線|连接|狀態|状态|通|connected|status|つなが|接続))", RegexOptions.IgnoreCase))
        {
            ...
            reply = available
                ? DialogueManagerUpdatePatch.ApiKeyText("本機目前有可用的網路連線。", "本机目前有可用的网络连接。", "今は利用できるネット接続があるよ。", "A network connection is currently available.")
                : DialogueManagerUpdatePatch.ApiKeyText("本機目前沒有偵測到可用的網路連線。", "本机目前没有检测到可用的网络连接。", "今は利用できるネット接続が見つからない。", "No available network connection is currently detected.");
            return true;
        }
        return false;
    }
    ...
    internal static bool TryOpenKnownFolder(string text, out string reply)
    {
        reply = string.Empty;
        ...
        if (Regex.IsMatch(text, "(截圖|截图|screenshot|スクリーンショット).{0,5}(資料夾|文件夹|folder|フォルダ)", RegexOptions.IgnoreCase))
        {
            ...
            name = DialogueManagerUpdatePatch.ApiKeyText("截圖資料夾", "截图文件夹", "スクリーンショットフォルダー", "Screenshots folder");
        }
        else if (Regex.IsMatch(text, "(下載|下载|downloads?|ダウンロード)", RegexOptions.IgnoreCase))
        {
            path = GetDownloadsDirectory();
            name = DialogueManagerUpdatePatch.ApiKeyText("下載資料夾", "下载文件夹", "ダウンロードフォルダー", "Downloads folder");
        }
        else if (Regex.IsMatch(text, "(桌面|desktop|デスクトップ)", RegexOptions.IgnoreCase))
        {
            path = Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory);
            name = DialogueManagerUpdatePatch.ApiKeyText("桌面資料夾", "桌面文件夹", "デスクトップフォルダー", "Desktop folder");
        }
        else if (Regex.IsMatch(text, "(文件(?:資料夾|文件夹)|文檔|文档|documents?|ドキュメント)", RegexOptions.IgnoreCase))
        {
            path = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
            name = DialogueManagerUpdatePatch.ApiKeyText("文件資料夾", "文档文件夹", "ドキュメントフォルダー", "Documents folder");
        }
        else if (Regex.IsMatch(text, "(圖片|图片|照片|pictures?|photos?|ピクチャ|写真).{0,5}(資料夾|文件夹|folder|フォルダ)?", RegexOptions.IgnoreCase))
        {
            ...
            name = DialogueManagerUpdatePatch.ApiKeyText("圖片資料夾", "图片文件夹", "ピクチャフォルダー", "Pictures folder");
        }
        else if (Regex.IsMatch(text, "(音樂|音乐|music|ミュージック).{0,5}(資料夾|文件夹|folder|フォルダ)?", RegexOptions.IgnoreCase))
        {
            path = Environment.GetFolderPath(Environment.SpecialFolder.MyMusic);
            name = DialogueManagerUpdatePatch.ApiKeyText("音樂資料夾", "音乐文件夹", "ミュージックフォルダー", "Music folder");
        }
        else if (Regex.IsMatch(text, "(影片|視頻|视频|videos?|ビデオ).{0,5}(資料夾|文件夹|folder|フォルダ)?", RegexOptions.IgnoreCase))
        {
            path = Environment.GetFolderPath(Environment.SpecialFolder.MyVideos);
            name = DialogueManagerUpdatePatch.ApiKeyText("影片資料夾", "视频文件夹", "ビデオフォルダー", "Videos folder");
        }
        else if (Regex.IsMatch(text, "(MOD|模組|模组).{0,5}(資料夾|文件夹|folder|フォルダ)", RegexOptions.IgnoreCase))
        {
            path = ModDataPaths.Directory;
            Directory.CreateDirectory(path);
            name = DialogueManagerUpdatePatch.ApiKeyText("MOD 資料夾", "MOD 文件夹", "MODフォルダー", "MOD folder");
        }
        else if (Regex.IsMatch(text, "(資源回收筒|回收站|recycle bin|ごみ箱)", RegexOptions.IgnoreCase))
        {
            path = "shell:RecycleBinFolder";
            name = DialogueManagerUpdatePatch.ApiKeyText("資源回收筒", "回收站", "ごみ箱", "Recycle Bin");
        }
        if (path == null)
            return false;
        ...
        try
        {
            ...
            reply = DialogueManagerUpdatePatch.ApiKeyText($"好，替你打開{name}。", $"好，替你打开{name}。", $"うん、{name}を開くね。", $"Okay, I'll open the {name}.");
            Plugin.PluginLog.LogInfo($"Opened allowlisted known folder '{name}'.");
        }
        catch (Exception exception)
        {
            reply = DialogueManagerUpdatePatch.ApiKeyText($"{name}沒有成功打開。", $"{name}没有成功打开。", $"{name}を開けなかった……", $"I couldn't open the {name}.");
    ...
    internal static bool TryHandleWindowCommand(string text, out string reply)
    {
        reply = string.Empty;
        string action;
        ...
        try
        {
            switch (action)
            {
                case "desktop":
                    ...
                    reply = DialogueManagerUpdatePatch.ApiKeyText("好，顯示桌面。", "好，显示桌面。", "うん、デスクトップを表示するね。", "Okay, showing the desktop.");
                    break;
                case "taskview":
                    SendShortcut(0x5B, 0x09);
                    reply = DialogueManagerUpdatePatch.ApiKeyText("工作檢視打開了。", "任务视图打开了。", "タスクビューを開いたよ。", "Task View is open.");
                    break;
                case "switch":
                    SendShortcut(0x12, 0x09);
                    reply = DialogueManagerUpdatePatch.ApiKeyText("替你切到上一個視窗。", "替你切到上一个窗口。", "前のウィンドウに切り替えたよ。", "I switched to the previous window.");
                    break;
                default:
                    var target = DialogueManagerUpdatePatch.GetControllableWindow();
                    if (target == IntPtr.Zero)
                        throw new InvalidOperationException("No external foreground window is available.");
                    if (action == "minimize")
                    {
                        ShowWindow(target, 6);
                        reply = DialogueManagerUpdatePatch.ApiKeyText("把剛才的視窗最小化了。", "把刚才的窗口最小化了。", "さっきのウィンドウを最小化したよ。", "I minimized the previous window.");
                    }
                    else if (action == "maximize")
                    {
                        ShowWindow(target, 3);
                        reply = DialogueManagerUpdatePatch.ApiKeyText("把剛才的視窗最大化了。", "把刚才的窗口最大化了。", "さっきのウィンドウを最大化したよ。", "I maximized the previous window.");
                    }
                    else if (action == "restore")
                    {
                        ShowWindow(target, 9);
                        reply = DialogueManagerUpdatePatch.ApiKeyText("視窗已經還原。", "窗口已经恢复。", "ウィンドウを元に戻したよ。", "I restored the window.");
                    }
                    else
                    {
                        ShowWindow(target, 9);
                        ...
                        reply = action == "left"
                            ? DialogueManagerUpdatePatch.ApiKeyText("把剛才的視窗排到左側了。", "把刚才的窗口排到左侧了。", "さっきのウィンドウを左側に並べたよ。", "I snapped the previous window to the left.")
                            : DialogueManagerUpdatePatch.ApiKeyText("把剛才的視窗排到右側了。", "把刚才的窗口排到右侧了。", "さっきのウィンドウを右側に並べたよ。", "I snapped the previous window to the right.");
                    }
                    break;
            }
        ...
        {
            reply = DialogueManagerUpdatePatch.ApiKeyText("這次沒有找到能操作的視窗。", "这次没有找到能操作的窗口。", "今回は操作できるウィンドウが見つからなかった。", "I couldn't find a window to control this time.");
    ...
    private static bool TryWriteClipboard(string text, out string reply)
    {
        ...
        {
            reply = DialogueManagerUpdatePatch.ApiKeyText("為了避免憑證外洩，我不會代為複製看起來像密碼、API Key 或驗證碼的內容。", "为了避免凭证泄露，我不会代为复制看起来像密码、API Key 或验证码的内容。", "認証情報の漏えいを避けるため、パスワード、APIキー、認証コードらしい内容はコピーしないよ。", "To avoid credential exposure, I won't copy text that looks like a password, API key, or verification code.");
            return true;
        }
        if (!EnsureAdvancedComputerActions(out reply))
            return true;
        ...
        reply = DialogueManagerUpdatePatch.ApiKeyText("已經替你複製到剪貼簿了。", "已经替你复制到剪贴板了。", "クリップボードにコピーしたよ。", "I copied it to the clipboard.");
    ...
    private static bool TryOpenBrowserSearch(string text, out string reply)
    {
            ...
            || (match = Regex.Match(text, "(?:ブラウザ|Google).{0,5}(?:で)?(?:検索|調べ)[：:\\s]*(?<query>.+)$", RegexOptions.IgnoreCase)).Success)
        {
            ...
            try
            {
                ...
                reply = DialogueManagerUpdatePatch.ApiKeyText("好，已經在瀏覽器搜尋了。", "好，已经在浏览器搜索了。", "うん、ブラウザで検索したよ。", "Okay, I searched for it in your browser.");
                Plugin.PluginLog.LogInfo($"Opened an explicit browser search ({query.Length} chars; query hidden from log).");
            }
            catch (Exception exception)
            {
                reply = DialogueManagerUpdatePatch.ApiKeyText("瀏覽器沒有成功打開。", "浏览器没有成功打开。", "ブラウザを開けなかった……", "I couldn't open the browser.");
    ...
    internal static bool TryHandleMediaCommand(string text, out string reply)
    {
        reply = string.Empty;
        ...
        {
            SendVirtualKey(key);
            reply = DialogueManagerUpdatePatch.ApiKeyText(traditional, simplified, japanese, english);
            Plugin.PluginLog.LogInfo($"Sent allowlisted Windows media key 0x{key:X2}.");
        }
        catch (Exception exception)
        {
            reply = DialogueManagerUpdatePatch.ApiKeyText("媒體控制沒有成功。", "媒体控制没有成功。", "メディア操作がうまくいかなかった……", "The media control did not work.");
